...
class Testimonial extends Gopanel {
	...
	public function add(){
		...
		if (isset($_POST['token'])) {
			...
			$_POST['image'] 	= file_upload($_FILES['image'],'/uploads/images/'.$this->table.'/',uniqid());

			if ($this->core->add_last_id($this->table,$_POST)) {
	...
	public function edit(){
		...
		if (isset($_POST['token'])) {
			...
			if (isset($_FILES['image']) && strlen($_FILES['image']['name'])>1) {
				$_POST['image'] = image_upload($_FILES['image'],'/uploads/images/'.$this->table.'/',uniqid());
			}
			else{
				unset($_POST['image']);
			}
